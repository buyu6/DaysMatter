...
private lateinit var dao: MessageDao
    ...
    private fun refreshCategoryMenu() {
        ...
        // 静态项id集合
        val staticIds = setOf(
            R.id.allEvent, R.id.lifeEvent, R.id.workEvent, R.id.missEvent, R.id.categoryEvent, R.id.historyEvent
        )
        // 找到“纪念日”和“分类”项的索引
        val missIndex = (0 until menu.size()).firstOrNull { menu.getItem(it).itemId == R.id.missEvent } ?: -1
        val categoryIndex = (0 until menu.size()).firstOrNull { menu.getItem(it).itemId == R.id.categoryEvent } ?: -1

        // 保存id、title和iconResId
        val staticAfterCategory = mutableListOf<Triple<Int, String, Drawable?>>()
        for (i in menu.size() - 1 downTo categoryIndex) {
            val item = menu.getItem(i)
            staticAfterCategory.add(Triple(item.itemId, item.title.toString(), item.icon))
            menu.removeItem(item.itemId)
        }

        // 移除“纪念日”与“分类”之间的所有项（动态分类）
